...
(function(){
  ...
  VoiceAnalyzer.prototype.getDatas = function(){
    ...
    setInterval(function(ev){
      ...
      if(this.deltaFFT) {
        ...
        $(window).trigger("fft_delta", [this.deltaFFT]);
      }
  ...
  VoiceAnalyzer.prototype.appendGraph = function(obj /* {"pcm", "fft", "sum", "fft_delta", "timestamp"} */) {
    ...
    var $sum = $("<div>")
      .append($("<span>").text("sum : "+obj.sum))
      .append("<br>")
      .append($("<label>").text("1").append($("<input>").attr({
        "type": "radio", 
        "name": "learn"+this.recordId,
        "value": 1,
        "class": "learn",
        "data-id": this.recordId
      })))
    ...
    $test.on("click", function(ev){
      var id = $(this).data("id");
      $(window).trigger("clicked", [{id: id, data: that.recordedDatas[id]}]);
    });
